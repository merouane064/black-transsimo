...
| Doubt about which version is live | Open `YOUR_/exec?action=ping` in a browser. It should reply `{"version":"2.14.4",...}`. If instead you get the bookings list, the Web App is still running old code. |
| New bookings not appearing in sheet / status resets to pending on refresh | **Root cause found:** Google Apps Script `/exec` answers writes with a **302 redirect** that drops a POST body; and a past `updateStatus` case bug meant status stopped syncing. The site now sends **URL query parameters** (which survive the redirect) and the script routes `action=create` / `action=updateStatus` / no action → list. **You must re-deploy a NEW VERSION** of `js/google-apps-script.js` for the fix to take effect (Deploy → Manage deployments → ✏️ on the existing Web app row → New version → Deploy → confirm `?action=ping` returns `2.14.4`). |
